...
class BaseEngine(ABC):
    def __init__(self, receipt_ocr_model: DefaultModel):
        if not isinstance(receipt_ocr_model, DefaultModel):
            raise TypeError
        self._model: DefaultModel = receipt_ocr_model
...
class DefaultEngine(BaseEngine):
    @timeit
    def inference_detector(self):
        output = infer_detector(self._model.cfg_detector, self._model.detector, self._input)
        self._out_detector = output

    @timeit
    def inference_recognizer(self):
        data_loader = data_preparation(opt=self._model.cfg_recognizer, list_data=self._imgs)
        pred, conf_score = infer_recognizer(
            opt=self._model.cfg_recognizer,
            model=self._model.recognizer,
    ...
    def combine_entity(self):
        ...
        for idx in range(len(self.raw_output) - 1):
            diff = abs(self.raw_output[idx][2][0] - self.raw_output[idx + 1][2][0])
            if diff < thres:
        ...
        for idx in range(len(all_entity)):
            all_entity[idx] = sorted(all_entity[idx], key=lambda x: (x[2][3], x[2][1], x[2][2]))
...
class ONNXEngine(DefaultEngine):
    @timeit
    def inference_detector(self):
        output = infer_detector(
            self._model.cfg_detector, self._model.detector, self._input, onnx=True
        )
        self._out_detector = output
